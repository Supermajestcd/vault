...
func parseExtValue(entry map[string]interface{}) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("'value' field value was not a string")
	}
...
func parseExtCritical(entry map[string]interface{}) (bool, error) {
	...
	if myBool, ok := critRaw.(bool); ok {
		return myBool, nil
	}

	critStr, ok := critRaw.(string)
	if !ok {
		return false, errors.New("critical field value was not a string or a bool")
	}

	if critStr == "" {
		// Optional field, so return false as a default value.
		return false, nil
	}

	return strconv.ParseBool(critStr)
}
...
func parseExtAsn1ObjectId(entry map[string]interface{}) (asn1.ObjectIdentifier, error) {
	...
	oidStr, ok := idRaw.(string)
	if !ok {
		return asn1.ObjectIdentifier{}, errors.New("id field value was not a string")
	}
...
func parseRevocationTime(cert map[string]interface{}) (time.Time, error) {
	...
	if revTimeInt, ok := revTimeRaw.(int); ok {
		// Probably got a Unix time format
		return time.Unix(int64(revTimeInt), 0), nil
	}

	revTimeStr, ok := revTimeRaw.(string)
	if !ok {
		return revTime, errors.New("'revocation_time' field value was not a string or int value")
	}

	// We ignore the error value here if set to see if we could parse it as an int later
	if parsedTime, err := time.Parse(time.RFC3339, revTimeStr); err == nil {
		return parsedTime, nil
	}

	intTime, err := strconv.Atoi(revTimeStr)
	if err != nil {
		return revTime, errors.New("failed to parse time as RFC3339 or unix time")
	}
...
func parseSerialNum(cert map[string]interface{}) (*big.Int, error) {
	...
	serialNumStr, serialNumOk := serialNumRaw.(string)
	if !serialNumOk {
		return nil, errors.New("'serial_number' field was not a string")
	}
